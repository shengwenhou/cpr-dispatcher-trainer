...
### 2026-07-14 補記四 — AEC 系統級回音消除（搶話黏段的正解）

實測：學員搶話與開場白 echo 黏成同一辨識段整段被濾。正解＝`inputNode.setVoiceProcessingEnabled(true)`（voice processing：AEC＋降噪）——喇叭輸出從麥克風訊號源頭消除，實測播放期間 RMS 0.08→<0.005、echo 零辨識，學員 barge-in 從此可乾淨辨識。啟用失敗自動退回座標 gate（雙保險保留）。`--no-aec` 供 A/B。**注意**：VP 模式下麥克風硬體格式會變（實測 16kHz/7ch），converter 惰性建立已相容；DJI 藍牙在 VP 模式的相容性以維護者實測為準（失敗有明確診斷）。
