...
| `S5Config` | `autoadvance_s` | `CPR_S5_AUTOADVANCE_S` | `4.0` | S5 每步沉默逾此秒數自動播下一步（取代 S5 的沉默 timeout reprompt；見 3.4）。 |
...
互動中可輸入 `/wait 12` 觀察分級 timeout（5 秒觸發 l1、10 秒觸發 l2）；輸入 `/state` 隨時檢視目前狀態與已填 slot；輸入 `/fault` 模擬技術故障看 `tech_fault` 台詞。**若目前正在 S5**，`/wait` 觸發的則是沉默 auto-advance（依賴時鐘推進，文字模式用虛擬時鐘模擬；預設逾 4 秒即推進下一個 sub-step）而非分級 timeout——S5 已排除在沉默 timeout 之外（見 3.4）；可反覆輸入 `/wait 5` 觀察擺位逐步自動往前帶，或直接說出當前 sub-step 的完成語（如「好了」）提前推進。
