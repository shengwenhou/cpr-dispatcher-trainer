...
- `CONSCIOUSNESS`：只有 `NO`（無意識/無反應）才通過續走 S4；`YES`（有反應）**不通過**——本課堂固定情境為無意識假人，學員若回報「有反應」屬與情境不符，引擎停在 S3 重問確認，不誤進 S4（真有意識的處置屬進階情境，v1 不涵蓋；詳見 3.4）。
...
**優先序**：在 S5 狀態下，`detect_all_positioned`（全局完成句，如「都擺好了」「手也放好了」）優先於 `detect_step_done` 判定——兩者字面可能重疊時，以語意更明確的全局完成句為準（見 3.5）。

**狀態相依語意**（`fsm.py` 的 `_handle_step_done`）：

| 當前狀態 | 行為 |
|---|---|
| S5 | 播下一個 sub-step（見 3.5） |
...
### 3.4 語境化極簡回應解讀（contextual short-answer resolution）

**動機**：單字／極簡回答（「好」「有」「不會」「我早就…了」）的意義完全由「當前問句」決定——同一個「有」，在 S3 問意識時是「有反應」，在 S4 問呼吸時是「有呼吸但未描述」，在 S5 指令後則是「做好了」。這類超短句字面資訊量極低，現有分類鏈（不論 LLM 或關鍵字）對其信心普遍不足，結果全數落到層 4（維護者於課堂試玩時的實測：一次課堂 log 中層 4 生成紀錄六筆，全部是這類極簡回答被誤判為 unknown 所致）。

**機制**：`fastpath.py` 新增 `resolve_short_answer(text, state)`，依「當前狀態」直接解讀超短句，命中即短路回傳對應 slot 或 `step_done`，不必等 LLM 往返。三個分類器同步支援：

- **`RegexFastPath.classify`**：在數數／結束訊號偵測之後、`detect_all_positioned`／`detect_step_done` 之前呼叫 `resolve_short_answer`，命中即回傳（來源標記 `regex_fastpath`）。
- **`KeywordFallbackClassifier.classify`**：開頭即呼叫同一函式（命中則把 `source` 改標為 `keyword_fallback` 後回傳），讓降級路徑也能處理單字回答，不必仰賴關鍵字比對這類低資訊量的句子。
- **`GeminiIntentClassifier`**：系統 prompt 第 6 點專門說明「單字/極簡回答依當前問句解讀」規則並附 few-shot 例句；`_build_prompt` 會依 `_STATE_QUESTION_HINT`（各狀態「當前問句在問什麼」的一句話描述）在 prompt 中注入語境提示，供 LLM 判斷時參照。

層 4（即時生成兜底）機制不變——三分類器都無法解讀的句子仍會落到層 4 或層 2，本機制只是大幅減少會落到那裡的極簡回答數量。

**per-state 映射表**：

| 類別 | 觸發詞／pattern | 狀態 | 結果 |
|---|---|---|---|
| 肯定詞（`好`／`好了`／`有`／`對`／`嗯`／`是`／`OK`／`可以`／`會`） | 整詞比對 | S5 | `step_done=true`（指令後回報完成→下一步） |
| | | S3（限「有／會／對／是」） | `consciousness=yes` |
| | | S4（限「有／對／是」） | `breathing=unclear`（有呼吸但未描述，觸發既有 probe 條件句追問） |
| 完成式（`早就…了`／`已經…了`／`…好了`／`…起來了`／`做完了`） | regex，非否定 | S5 | `step_done=true` |
| 否定詞（`不會`／`沒有`／`沒了`／`不行`／`叫不醒`／`沒反應`） | 整詞比對 | S3 | `consciousness=no` |
| | | S4 | `breathing=absent` |

（S1／S2／S6 狀態下的極簡回答目前無明確 slot 語意，一律回 `None` 交由後續分類處理。）

**誤觸防範**（實作於 `resolve_short_answer`／`_normalize_short`／`_is_short`）：

- 只有「整句去尾語氣詞與標點後（`_TRAILING_PARTICLES`）＝該詞」才短路——採**整詞匹配**且核心字串長度須 ≤4 字（`_is_short`），所以「好痛」「有人嗎」「有點喘」去尾後仍非整詞，不會誤命中。
- 完成式 pattern（`_COMPLETION_RE`）不受長度限制，但比對前須先排除句中含「不／沒／別」（避免「已經沒呼吸了」這種否定描述被誤判為完成確認）。
- 「都擺好了」「手也放好了」屬**全局完成**語意（對應 S5 的 `detect_all_positioned`，跳過剩餘步驟直達 S6），語意更明確，優先於完成式的單步 `step_done`——`resolve_short_answer` 內完成式分支會先讓路給 `detect_all_positioned`。
- 否定判斷優先於肯定判斷（否定詞清單先比對）。
- 對意識 yes-no 問句（S3），單獨的「好」「嗯」「OK」語意不明確（無法判斷是「回答有反應」還是單純口頭語氣），刻意回傳 `None` 交由 LLM 判斷，不在 fastpath 層硬猜。

**CONSCIOUSNESS=YES 的處理變更**：承 3.2 所述，`slot_satisfies` 現在只認 `NO` 通過 S3 gating；`YES`（有反應）**不通過**——本課堂固定為無意識假人情境，學員若回報「有反應」屬與情境不符的例外回報。此時引擎（`_advance_from_current` 步驟 0：gating slot「已填但不滿足」時，有條件句就播、沒有條件句就重問當前問句）會停在 S3 重問確認，不會誤判為「已評估、可以放行」而誤進 S4，也不會因無對應處理路徑而發生非預期行為。
...
- **層 4 的生成語境與禁令**：生成 prompt（`llm_gemini.py` 的 `_LAYER4_SYSTEM`）現在會帶入「狀態語境」`STATE_CONTEXT_FOR_GEN`（`intents.py`）——明確告知生成器當前狀態下哪些動作**尚未開始**（例如 S5 擺位階段的語境句明示「學員尚未開始壓胸，此刻絕不可提到壓胸或數數」），並列出三條硬性禁令：不可提及尚未開始的操作、不可複誦問句（系統會在生成句之後自動重問當前問句）、只能安撫承接拉回。生成器呼叫簽名相應改為 `generate(utterance, question, state_context)`（`Layer4Generator` 型別對齊為 `Callable[[str, str, str], Optional[str]]`）。
- **生成後驗證層**：引擎在 `_handle_unknown` 取得生成結果後，先經 `.strip()[:max_chars]` 截斷，再呼叫 `layer4_text_violates(text, state)`（`intents.py`）做二次把關——以 `PROCEDURE_ACTION_KEYWORDS` 表（動作關鍵詞 → 允許提及的狀態集合；如「壓胸」「按壓」「數」只允許 `S6`，`AED`／人工呼吸／電擊／翻身等關鍵詞的允許集合為空，代表本課堂任何狀態都不許由層 4 提及）檢查生成文字是否含「當前狀態不該提及的流程動作」。命中即判違規：丟棄該次生成、改降級走層 2（保守優先），並記一筆 metrics（`EventType.DEFENSE`，`layer=4`，`rejected=True`，`reason="procedure_leak"`，`keyword=<命中的關鍵詞>`）供事後檢視。這修正了層 4 過去可能在 S5 擺位階段生成「繼續壓胸」之類超前指示的問題——即使 prompt 端的禁令沒有完全生效，驗證層仍會攔下。
- 每次生成（不論是否被驗證層丟棄）皆會寫入 `logs/layer4/`（見第六節設定）供課後審核，好的轉正為 FAQ 台詞。
...
**`GeminiIntentClassifier`**（`llm_gemini.py`）：走 Vertex AI（`google-genai`，`vertexai=True`），認證透過標準環境變數 `GOOGLE_APPLICATION_CREDENTIALS`（ADC），程式碼本身不引用金鑰內容或路徑。呼叫 `generate_content` 時帶 `response_mime_type="application/json"` 與明確的 `response_schema`（`_INTENT_SCHEMA`），做 constrained decoding，一次回傳多個 slot 欄位＋`confidence`＋可選的 `faq_id`／`end_signal`／`step_done`。系統 prompt 明確要求「只做分類、不生成台詞」，並特別提示諧音容錯（電話報案語境常見「救護人員」被辨識成「客戶人員」等）。`available()` 會檢查環境變數是否存在並嘗試建立 client，結果快取避免重複探測；`classify_intent` 任何例外（逾時／認證失敗／內容過濾）皆捕捉並回傳空的低信心結果，不拋例外中斷對話。同檔另有 `GeminiLayer4Generator`，是層 4 即時生成的實作，重用同一個 `GeminiIntentClassifier` 的 client／認證。呼叫簽名為 `generate(utterance, question, state_context)`（`__call__` 對齊同簽名）；`state_context` 由引擎依當前狀態從 `STATE_CONTEXT_FOR_GEN` 取得後傳入，prompt（`_LAYER4_SYSTEM`）將此語境與「不提尚未開始的操作／不複誦問句／只安撫承接拉回」三條禁令一併嵌入。每次生成（連同 `state_context`）寫入 `log_dir` 一個 JSON 檔待審核；生成文字是否「超前流程」的最終把關在引擎端的 `layer4_text_violates`（見第四節），本生成器只負責在 prompt 給足約束並產文。
...
| `S5Config` | `autoadvance_s` | `CPR_S5_AUTOADVANCE_S` | `4.0` | S5 每步沉默逾此秒數自動播下一步（取代 S5 的沉默 timeout reprompt；見 3.5）。 |
...
| 指令 | 說明 |
|---|---|
| `/wait <秒>` | 推進虛擬時鐘 N 秒並週期 pump `tick`（模擬沉默觸發分級 timeout、S5 沉默 auto-advance；S6 插播計時），預設 5 秒，每 0.5 秒 pump 一次。 |
...
互動中可輸入 `/wait 12` 觀察分級 timeout（5 秒觸發 l1、10 秒觸發 l2）；輸入 `/state` 隨時檢視目前狀態與已填 slot；輸入 `/fault` 模擬技術故障看 `tech_fault` 台詞。**若目前正在 S5**，`/wait` 觸發的則是沉默 auto-advance（依賴時鐘推進，文字模式用虛擬時鐘模擬；預設逾 4 秒即推進下一個 sub-step）而非分級 timeout——S5 已排除在沉默 timeout 之外（見 3.5）；可反覆輸入 `/wait 5` 觀察擺位逐步自動往前帶，或直接說出當前 sub-step 的完成語（如「好了」）提前推進。
